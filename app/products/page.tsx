...
export default function Page() {
  ...
  // TODO: Добавить скелетоны чтобы была анимация. Создай компонент Skeleton и прокинь туда пропсы чтобы можно было задавать размеры.
  if (isLoading) {
    return (
      <div className="px-2 py-2 items-center font-light">
        Loading posts ....{" "}
      </div>
    );
  }
  // Добавить страницу ошибки и обрабатывать ее
  if (error) {
  ...
  return (
    <>
      <div className="w-full relative flex items-center justify-center px-5 py-5 border-b border-gray-200">
        ...
        >
        {/* TODO: Иконки можно вынести в svg sprite */}
      ...
      <div className="px-5 py-6 columns-8  gap-4 space-y-4">
        {/* TODO: Вынести карточку продукта в отдельный компонент */}
        {posts
          ?.filter((post) => (filteredPosts ? post.liked : true))
          .map((post) => (
            <Link href={`/products/${post.id}`} key={post.id}>
              ...
              >
                ...
                <p className="text-black h-32 overflow-hidden text-sm mb-3">
                  <span className="text-red-400 mr-1">Body:</span>
                  {post.body}
                </p>
                ...
                {/* TODO: Компонент кнопки вынести в отдельный компонент и например назвать ее FavoriteButton */}
                <button
